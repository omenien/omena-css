...
interface EngineDocumentSchema {
  readonly $ref?: string;
  readonly type?: string;
  readonly properties?: Readonly<Record<string, EngineDocumentSchema>>;
  readonly required?: readonly string[];
  readonly items?: EngineDocumentSchema;
  readonly allOf?: readonly EngineDocumentSchema[];
  readonly anyOf?: readonly EngineDocumentSchema[];
}

// Read the emitted IDL graph rather than maintaining a second document schema
// in a Rust template. Unsupported schema constructs fail generation explicitly.
function renderRustEngineDocumentInputs(): string {
  const definitions = new Map<string, EngineDocumentSchema>();
  const read = (name: string): EngineDocumentSchema => {
    if (!definitions.has(name)) {
      assert.match(name, /^[A-Za-z][A-Za-z0-9]*$/u);
      definitions.set(
        name,
        JSON.parse(fs.readFileSync(path.join(schemaDir, `${name}.json`), "utf8")),
      );
    }
    return definitions.get(name)!;
  };
  const referenceName = (reference: string): string => {
    assert.match(reference, /^[A-Za-z][A-Za-z0-9]*\.json$/u);
    return reference.slice(0, -5);
  };
  const rustType = (schema: EngineDocumentSchema): string => {
    if (schema.$ref) {
    ...
    if (schema.anyOf) {
      const nonNull = schema.anyOf.filter((entry) => entry.type !== "null");
      assert.equal(schema.anyOf.length, 2, "only explicit nullable document unions are supported");
      assert.equal(nonNull.length, 1, "document unions must have exactly one null arm");
    ...
    switch (schema.type) {
      ...
      case "boolean":
        return "bool";
      case "array":
        assert.ok(schema.items, "document arrays require typed items");
        return `Vec<${rustType(schema.items)}>`;
      default:
        throw new Error(`unsupported EngineInput document schema: ${JSON.stringify(schema)}`);
    }
  };
  const objectFields = (
    schema: EngineDocumentSchema,
  ): {
    properties: Record<string, EngineDocumentSchema>;
    required: Set<string>;
  } => {
    assert.equal(schema.type, "object");
    const properties: Record<string, EngineDocumentSchema> = {};
    const required = new Set<string>();
    for (const parent of schema.allOf ?? []) {
      assert.ok(parent.$ref, "document inheritance must reference a named IDL model");
      const inherited = objectFields(read(referenceName(parent.$ref)));
  ...
  read("SourceAnalysisInputV2");
  read("StyleAnalysisInputV2");
  const rendered = new Map<string, string>();
  // Referenced models are added while their parent fields are rendered.
  for (const [name, schema] of definitions) {
    if (schema.type !== "object") {
      rendered.set(name, `pub type ${name}Json = ${rustType(schema)};`);
      continue;
    }
    const { properties, required } = objectFields(schema);
    const fields = Object.entries(properties).map(([field, shape]) => {
      assert.match(field, /^[A-Za-z][A-Za-z0-9]*$/u);
      const fieldName = field.replace(/([a-z0-9])([A-Z])/gu, "$1_$2").toLowerCase();
      const fieldType = rustType(shape);
      const optional = !required.has(field);
      const attributes = optional
        ? '    #[serde(skip_serializing_if = "Option::is_none")]\n'
        : fieldType.startsWith("Option<")
          ? '    #[serde(deserialize_with = "deserialize_required_nullable")]\n'
          : "";
      return `${attributes}    pub ${fieldName}: ${optional ? `Option<${fieldType}>` : fieldType},`;
    });
    rendered.set(
      name,
...
${renderRustEngineDocumentInputs()}
