...
function resolvePassIds(options) {
  ...
  if (options.treeShake) {
    appendPassIds(passIds, TREE_SHAKE_PASS_IDS);
  }
  if (options.bundle) {
    appendPassIds(passIds, BUNDLE_PASS_IDS);
  }
  if (options.minify) {
    appendPassIds(passIds, MINIFY_PASS_IDS);
  }
  return passIds;
}
...
async function loadOmenaEngine(options) {
  ...
  throw new Error(
    "[omena-css] Unable to load @omena/napi or @omena/wasm. Install @omena/napi or enable the wasm package; CLI/cargo fallback is intentionally not used on the Vite hot path.",
  );
}
