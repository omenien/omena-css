...
#[test]
fn resolves_html_template_class_definition_to_opened_style_selector() -> TestResult {
...
"#;
    ...
    let source_uri = "file:///workspace-a/src/Page.html";
    ...
    let document = state
        .document(source_uri)
        .ok_or_else(|| std::io::Error::other("HTML document is open"))?;
    assert!(
        document
            .source_syntax_index
            .selector_references
        ...
        json!({
            ...
            "params": {
                ...
                "position": parser_position_for_byte_offset(
                    source,
                    fixture_find(source, "root", "HTML template contains root class")? + 1,
                ),
            },
        }),
    );
