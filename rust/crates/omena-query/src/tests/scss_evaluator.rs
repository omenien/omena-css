...
fn exposes_static_stylesheet_oracle_corpus_through_query_boundary() {
    ...
    assert_eq!(summary.value_type, "AbstractCssValueV0");
    assert_eq!(summary.product_output_source, "nativeEditOutput");
    assert_eq!(summary.fixture_count, 139);
    assert_eq!(summary.scss_fixture_count, 48);
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "scss.dynamic-mixin-local")
    );
    assert!(
        summary
            .corpus
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "scss.recursive-nested-mixin-include")
    );
    assert!(
        summary
            .corpus
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.unknown-detached-ruleset-mixin-call")
    );
    assert!(
        summary
            .corpus
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.recursive-nested-mixin-call")
    );
    assert!(
        summary
            .corpus
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.named-mixin-arguments")
    );
    assert!(
        summary
            .corpus
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.semicolon-named-mixin-arguments")
    );
    assert!(
        summary
            .corpus
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.unknown-mixin-accessor-property-member")
    );
    assert!(
        summary
            .corpus
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.unknown-mixin-call-suffix")
    );
    assert!(
        summary
            .corpus
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.property-variable-alias")
    );
    assert!(
        summary
            .corpus
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.isdefined-guarded-mixin")
    );
    assert!(
        summary
            .corpus
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "less.property-guarded-mixin")
    );
    assert!(
        summary
            .corpus
...
fn exposes_scss_control_flow_oracle_corpus_through_query_boundary() {
    ...
    assert_eq!(summary.supported_fixture_count, 68);
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "scss.static-for-expression-bounds"
                && fixture.call_resolved_return_value_count == 1
                && fixture.value_analysis_converged)
    );
    assert!(
    ...
    assert!(summary.corpus.fixtures.iter().any(|fixture| fixture.id
        ...
        && fixture.value_analysis_converged));
    assert!(summary.corpus.fixtures.iter().any(|fixture| fixture.id
        == "sass.static-for-finite-set-bound"
        && fixture.dialect == "sass"
        && fixture.loop_block_count == 2
        && fixture.back_edge_count == 2
        && fixture.value_analysis_converged));
    ...
    assert!(summary.corpus.fixtures.iter().any(|fixture| fixture.id
        ...
        && fixture.value_analysis_converged));
    assert!(summary.corpus.fixtures.iter().any(|fixture| fixture.id
        == "sass.static-while-finite-set-bound"
        && fixture.dialect == "sass"
        && fixture.loop_block_count == 2
        && fixture.back_edge_count == 2
        && fixture.value_analysis_converged));
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "sass.static-each-return"
                && fixture.dialect == "sass"
                && fixture.call_resolved_return_value_count == 1
                && fixture.value_analysis_converged)
    );
    ...
    assert!(
        summary
            ...
            .fixtures
            .iter()
            .any(|fixture| fixture.id == "sass.branch-if-else"
                && fixture.dialect == "sass"
    ...
    for id in [
        ...
        assert!(
            ...
            "query boundary must expose nested mixin call edge oracle fixture {id}"
        );
    }
    assert!(
        summary
...
#[test]
fn exposes_sass_static_stylesheet_evaluator_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "$gap: 1px\n.card\n  margin: $gap",
...
#[test]
fn exposes_dynamic_scss_function_calls_as_preserved_oracle_output() {
...
#[test]
fn exposes_static_scss_dynamic_mixin_locals_as_preserved_oracle_output_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert_eq!(summary.native_replacement_count, 0);
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains("@mixin tone")
            && evaluation.evaluated_css.contains("meta.inspect((a: b))")
            && evaluation.evaluated_css.contains("@include tone")
            && !evaluation.evaluated_css.contains("margin: (a: b)")
    }));
}

#[test]
fn exposes_static_scss_mixin_if_blocks_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert_eq!(summary.divergence_count, 0);
    assert!(summary.all_legacy_declaration_values_preserved);
    assert!(summary.native_structural_edit_count >= 2);
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
...
#[test]
fn exposes_static_sass_mixin_if_expansion_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "@mixin tone($enabled)\n  @if $enabled\n    color: red\n  @else\n    color: blue\n.button\n  @include tone(false)",
        OmenaParserStyleDialect::Sass,
    );
...
#[test]
fn exposes_static_sass_mixin_each_expansion_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
...
#[test]
fn exposes_static_sass_mixin_while_expansion_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "@mixin items()\n  $i: 0\n  @while $i < 3\n    $i: $i + 1\n    order: $i\n.button\n  @include items()",
        OmenaParserStyleDialect::Sass,
    );
...
#[test]
fn exposes_static_scss_top_level_if_blocks_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        !evaluation.evaluated_css.contains("@if")
            ...
            && evaluation.evaluated_css.contains(".off { color: red; }")
            && !evaluation.evaluated_css.contains(".on { color: green; }")
            && evaluation.native_edit_output_matches_evaluated_css
    }));
}

#[test]
fn exposes_dynamic_scss_top_level_if_blocks_as_preserved_oracle_through_query_boundary() {
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css == source
            && evaluation.native_edit_count == 0
            && evaluation.native_edit_output_matches_evaluated_css
    }));
}

#[test]
fn exposes_static_scss_top_level_if_value_replacements_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        !evaluation.evaluated_css.contains("@if")
            && !evaluation.evaluated_css.contains("@else")
            && !evaluation.evaluated_css.contains("$brand")
...
#[test]
fn exposes_static_scss_top_level_for_expansion_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
...
#[test]
fn exposes_static_scss_top_level_while_expansion_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
...
#[test]
fn exposes_static_sass_top_level_if_value_expansion_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "$enabled: true\n$brand: green\n@if $enabled\n  .on\n    color: $brand\n@else\n  .off\n    color: gray",
        OmenaParserStyleDialect::Sass,
    );
...
#[test]
fn exposes_static_sass_top_level_if_function_expansion_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "@function tone($color)\n  @return $color\n$enabled: true\n@if $enabled\n  .on\n    color: tone(green)\n@else\n  .off\n    color: gray",
        OmenaParserStyleDialect::Sass,
    );
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        !evaluation.evaluated_css.contains("@function")
            && !evaluation.evaluated_css.contains("@if")
            && !evaluation.evaluated_css.contains("@else")
            && !evaluation.evaluated_css.contains("tone(green)")
            && evaluation.evaluated_css.contains("color: green")
            && !evaluation.evaluated_css.contains("color: gray")
            && evaluation.native_edit_output_matches_evaluated_css
    }));
...
#[test]
fn exposes_static_scss_recursive_nested_mixin_includes_as_preserved_oracle_output_through_query_boundary()
 {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert_eq!(summary.native_replacement_count, 0);
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains("@mixin a")
            && evaluation.evaluated_css.contains("@mixin b")
            && evaluation.evaluated_css.contains("@include a")
            && evaluation.evaluated_css.contains("@include b")
    }));
}
...
#[test]
fn exposes_less_property_name_interpolation_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains("color: red")
            && !evaluation.evaluated_css.contains("@{prop}")
            && !evaluation.evaluated_css.contains("@prop:")
    }));
}

#[test]
fn exposes_less_simple_selector_interpolation_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert_eq!(summary.native_structural_edit_count, 2);
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains(".card")
...
#[test]
fn exposes_less_chained_class_selector_interpolation_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains(".card-primary")
...
#[test]
fn exposes_less_chained_id_selector_interpolation_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "@prefix: card; @suffix: primary; #@{prefix}-@{suffix} { color: red; }",
        OmenaParserStyleDialect::Less,
    );
...
#[test]
fn exposes_less_variable_indirection_aliases_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "@name: color; @color: red; @alias: @@name; .card { color: @alias; }",
        OmenaParserStyleDialect::Less,
    );
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains("color: red")
            && !evaluation.evaluated_css.contains("@alias")
            && !evaluation.evaluated_css.contains("@@name")
            && !evaluation.evaluated_css.contains("@name:")
            && !evaluation.evaluated_css.contains("@color:")
    }));
...
#[test]
fn exposes_less_recursive_nested_mixin_calls_as_preserved_oracle_output_through_query_boundary() {
...
#[test]
fn exposes_less_property_variable_aliases_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        ".button { margin: (1px + 2px); color: red; @gap: $margin; @outline: 1px solid $color; padding: @gap; border: @outline; }",
        OmenaParserStyleDialect::Less,
    );
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains("padding: 3px")
            && evaluation.evaluated_css.contains("border: 1px solid red")
            && evaluation.resolved_replacements.iter().any(|replacement| {
                replacement.name == "@gap"
                    && replacement.text == "3px"
                    && replacement.abstract_value_kind == "exact"
            })
            && evaluation.resolved_replacements.iter().any(|replacement| {
...
#[test]
fn exposes_less_isruleset_builtin_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains("ok: true")
            && evaluation.evaluated_css.contains("bad: false")
    }));
}
...
#[test]
fn exposes_less_named_mixin_arguments_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        !evaluation.evaluated_css.contains(".tone(@color")
            && !evaluation.evaluated_css.contains(".tone(@gap")
            && evaluation.evaluated_css.contains("color: blue")
            && evaluation.evaluated_css.contains("margin: 2px")
    }));
}
...
#[test]
fn exposes_less_semicolon_named_mixin_arguments_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        ".tone(@color; @gap: 1px) { color: @color; margin: @gap; } .button { .tone(@gap: 2px; @color: blue); }",
        OmenaParserStyleDialect::Less,
    );
...
#[test]
fn exposes_less_variadic_mixin_arguments_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.all_legacy_declaration_values_preserved);
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        !evaluation.evaluated_css.contains(".shadow(@color")
            && !evaluation.evaluated_css.contains(".shadow(red")
            && evaluation.evaluated_css.contains("color: red")
            && evaluation
                .evaluated_css
...
#[test]
fn exposes_less_unknown_mixin_accessor_members_as_preserved_oracle_output_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
...
#[test]
fn exposes_less_unknown_mixin_accessor_property_members_as_preserved_oracle_output_through_query_boundary()
 {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        ".tokens(@color) { result: @color; } .button { color: .tokens(red)[missing]; }",
        OmenaParserStyleDialect::Less,
...
#[test]
fn exposes_less_unmatched_literal_pattern_mixins_as_preserved_oracle_output_through_query_boundary()
{
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert_eq!(summary.native_replacement_count, 0);
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains(".tone(light, red);")
            && evaluation.evaluated_css.contains(".tone(dark, @color)")
            && !evaluation.evaluated_css.contains(".button { color: red")
    }));
}
...
#[test]
fn exposes_less_unknown_mixin_call_suffixes_as_preserved_oracle_output_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert_eq!(summary.native_replacement_count, 0);
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains(".tone(red) !default;")
            && evaluation.evaluated_css.contains(".tone(@color)")
            && !evaluation.evaluated_css.contains(".button { color: red")
    }));
}
...
#[test]
fn exposes_less_decidable_negated_default_guarded_mixins_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation.evaluated_css.contains("color: red")
            && !evaluation.evaluated_css.contains("color: blue")
            && !evaluation.evaluated_css.contains(".tone()")
            && !evaluation.evaluated_css.contains("not(default())")
    }));
}
...
#[test]
fn exposes_less_isdefined_guarded_mixin_oracle_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
        "@brand: red; .present() when (isdefined(@brand)) { color: @brand; } .with-param(@tone) when (isdefined(@tone)) { border-color: @tone; } .button { .present(); .with-param(green); }",
        OmenaParserStyleDialect::Less,
    );
...
#[test]
fn exposes_less_property_guarded_mixins_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        !evaluation.evaluated_css.contains(".space()")
            && !evaluation.evaluated_css.contains(".tone()")
            && !evaluation.evaluated_css.contains(".unit()")
            && evaluation.evaluated_css.contains("padding: 3px")
            && evaluation.evaluated_css.contains("border-color: red")
            && evaluation.evaluated_css.contains("inset: 4px")
    }));
...
#[test]
fn exposes_static_scss_each_function_source_returns_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
...
#[test]
fn exposes_static_scss_each_tuple_function_source_returns_through_query_boundary() {
    let summary = summarize_omena_query_static_stylesheet_evaluator_from_source(
    ...
    assert!(summary.evaluation.as_ref().is_some_and(|evaluation| {
        evaluation
            ...
            && evaluation.resolved_replacements.iter().any(|replacement| {
                replacement.name == "function:width-for"
                    && replacement.text == "2px"
                    && replacement.abstract_value_kind == "exact"
            })
    }));
}
...
#[test]
fn exposes_sass_static_while_finite_set_bound_bindings_through_query_boundary()
...
"#;
    ...
    let while_block = value_analysis
        .blocks
        .iter()
        .find(|block| block.loop_carried_bindings == vec!["$i"]);
    assert!(while_block.is_some());
    let Some(while_block) = while_block else {
        return Ok(());
    };
    assert_eq!(
        serde_json::to_value(&while_block.loop_carried_binding_values[0].value)?,
        serde_json::json!({
            "kind": "finiteSet",
            "values": ["0", "1", "2", "3"],
        })
...
#[test]
fn exposes_sass_static_for_finite_set_bound_bindings_through_query_boundary()
-> Result<(), serde_json::Error> {
    let source = r#"
@each $end in 2, 4
...
#[test]
fn exposes_static_while_finite_set_bound_bindings_through_query_boundary()
-> Result<(), serde_json::Error> {
    let source =
        "@each $limit in 2, 4 { $i: 0; @while $i < $limit { $i: $i + 1; .n { order: $i; } } }";
...
#[test]
fn exposes_descending_static_for_conditional_call_return_values_through_query_boundary()
-> Result<(), serde_json::Error> {
    let source = "@function collect($target) { @for $i from 3 through 1 { @if $i == $target { @return $i; } } @return 0; } .a { width: collect(2); }";
...
#[test]
fn exposes_static_for_expression_bound_call_return_values_through_query_boundary()
-> Result<(), serde_json::Error> {
...
#[test]
fn exposes_static_each_function_source_call_return_values_through_query_boundary()
-> Result<(), serde_json::Error> {
...
#[test]
fn exposes_static_each_tuple_function_source_call_return_values_through_query_boundary()
-> Result<(), serde_json::Error> {
...
#[test]
fn exposes_native_css_function_cycle_through_query_boundary() {
    let source =
    ...
    assert_eq!(summary.product, "omena-query.native-css-evaluator");
    assert!(summary.native_function_call_count >= 1);
    assert_eq!(summary.native_function_call_foldable_count, 0);
    assert!(summary.native_function_call_structural_error_count >= 1);
    assert_eq!(summary.native_static_edit_count, 0);
    assert!(!summary.native_static_edit_output_changed);
    assert!(
        summary
