...
const contextStyleSource =
  '@import "./tokens.css"; .button { composes: base; color: var(--brand); } .base { color: blue; } .button :global(.external) { color: var(--brand); } @media (min-width: 1px) { .button { color: var(--brand); } }';
...
assert.equal(contextSummary.execution.mutationCount, 9);
