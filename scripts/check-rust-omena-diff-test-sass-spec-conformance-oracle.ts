...
const chunkPath = path.join(corpusRoot, "conformance-smoke.json");
...
assert.equal(chunk.product, "omena-diff-test.sass-spec-conformance-corpus.chunk");
assert.ok(chunk.fixtures.length > 0, "conformance sass-spec chunk must not be empty");
...
assert.equal(
  ...
  sha256(secondSource),
  "dart-sass conformance oracle capture must be deterministic for the same chunk",
);
assert.ok(
  primaryCapture.records.every((record) => record.compiled),
  "every conformance corpus fixture must compile through pinned dart-sass",
);
assert.ok(
  primaryCapture.records.every((record) => (record.declarationValuePairs?.length ?? 0) > 0),
  "every conformance corpus fixture must yield declaration value pairs",
);
...
process.stdout.write(
  stableJson({
    product: "omena-diff-test.sass-spec-conformance-dart-sass-oracle",
...
function captureChunk(chunk: ConformanceSassSpecChunkV0): SassSpecOracleCaptureV0 {
  return {
    schemaVersion: "0",
    product: "omena-diff-test.sass-spec-conformance-dart-sass-oracle-capture",
    sourcePin: chunk.sourcePin,
    compiler: {
      name: "dart-sass",
      package: "sass",
...
function captureFixture(fixture: ConformanceSassSpecFixtureV0): SassSpecOracleRecordV0 {
  const outputRoot = mkdtempSync(path.join(tmpdir(), "omena-sass-conformance-oracle-"));
  ...
  try {
    ...
    assert.equal(result.status, 0, `${fixture.id} must compile: ${result.stderr}`);
