...
const contextStyleSource =
  '@import "./tokens.css"; .button { composes: base; color: var(--brand); } .base { color: blue; } .button :global(.external) { color: var(--brand); } @media (min-width: 1px) { .button { composes: base; color: var(--brand); } }';
...
assert.equal(
  ...
  ":root { --brand: red; } ._button_x{  color: var(--theme-brand); } ._base_y{ color: blue; } ._button_x :global(.external){ color: var(--theme-brand); } @media (min-width: 1px) { ._button_x{  color: var(--theme-brand); } }",
);
...
assert.equal(contextSummary.execution.mutationCount, 10);
...
assert.ok(
  semanticReachabilitySummary.execution.semanticRemovals.some(
    (removal) =>
      removal.passId === "tree-shake-custom-property" &&
      removal.symbolKind === "customPropertyRegistration" &&
      removal.name === "--dead",
  ),
