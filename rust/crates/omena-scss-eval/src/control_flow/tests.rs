...
#[test]
fn native_css_when_else_routes_through_edge_ir_without_pruning_unknown_conditions() {
    let source = "@when media(screen) { .on { color: green; } } @else { .off { color: red; } }";
    ...
    assert_eq!(report.branch_block_count, 2);
    assert_eq!(report.loop_block_count, 0);
    assert!(report.flat_css_cfg_built);
    assert_eq!(report.blocks[0].at_rule_name, "@when");
    assert_eq!(report.blocks[0].kind, "branchIf");
    assert_eq!(report.blocks[1].at_rule_name, "@else");
    assert_eq!(report.blocks[1].kind, "branchElse");

    assert_eq!(graph.dialect, "css");
    ...
    assert!(analysis.flat_css_cfg_built);
    assert!(
        analysis
            .blocks
            .iter()
            .all(|block| block.transfer_truthiness.is_none())
    );

    assert_eq!(prune.dialect, "css");
...
#[test]
fn native_css_if_rule_supports_routes_static_truthiness_through_edge_ir() {
    let source =
        "@if supports(display: grid) { .on { color: green; } } @else { .off { color: red; } }";
    let report = summarize_scss_control_flow_ir(source, StyleDialect::Css);
    let analysis = analyze_scss_control_flow_values(source, StyleDialect::Css);
    let prune = summarize_scss_control_flow_prune_reachability(source, StyleDialect::Css);
    assert!(report.is_some());
...
#[test]
fn call_return_ir_reports_mixin_content_block_edges() {
    let source = "@mixin tone($color) { @content; color: $color; } .a { @include tone(red) { background: white; } }";
    let report = summarize_scss_call_return_ir(source, StyleDialect::Scss);
    assert!(report.is_some());
    let Some(report) = report else {
        return;
