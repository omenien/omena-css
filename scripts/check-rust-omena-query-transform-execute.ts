...
assert.equal(
  ...
  ".card { width: 4px; height: 2rem; margin: min(1px, 1rem); opacity: 0.5; outline-width: 6px; flex-basis: 24px; inline-size: 4px; line-height: 0.5; stroke-width: 2px; order: -1; top: 9px; right: 12px; bottom: 9px; left: 9px; rotate: round(nearest, 5px, 2px); }",
);
assert.deepEqual(mathFunctionReductionSummary.execution.executedPassIds, [
  "calc-reduction",
  "print-css",
]);
assert.equal(mathFunctionReductionSummary.execution.mutationCount, 13);
