...
#[test]
fn resolves_astro_frontmatter_css_module_definition_to_opened_style_selector() -> TestResult {
    let source = r#"---
...
"#;
    ...
    let document = state
        .document(source_uri)
        .ok_or_else(|| std::io::Error::other("Astro document is open"))?;
    assert_eq!(
        document
            .source_syntax_index
        ...
        json!({
            ...
            "params": {
                ...
                "position": parser_position_for_byte_offset(
                    source,
                    fixture_find(source, "styles.root", "Astro source contains styles.root")?
                        + "styles.".len()
                        + 1,
                ),
            },
