...
describe("check orchestrator manifest", () => {
  ...
  it("surfaces declared replacements as preserved compatibility scripts", () => {
    ...
    expect(inventory).toMatch(
      /\| `release\/release\/verify` \| bundle \| declared \|[^|]*\|[^|]*\| `release:verify` \|[^|]*\| compatibility script;/,
    );
    expect(inventory).toMatch(
      /\| `rust\/lane\/bundle` \| bundle \| declared \|[^|]*\|[^|]*\| `check:rust-lane-bundle` \|[^|]*\| compatibility script;/,
    );
  });
    ...
    expect(diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          message: expect.stringContaining("exceeds the governed maximum 156"),
        }),
      ]),
    );
  });
  ...
  it("renders a deterministic check inventory", () => {
    ...
    expect(inventory).toMatch(
      /\| ID\s+\| Kind\s+\| Origin\s+\| Cadence\s+\| Strength\s+\| Script\s+\| References\s+\| Status\s+\|/,
    );
    expect(inventory).toMatch(
      /\| `tsgo\/release\/bundle` \| alias \| package \|[^|]*\|[^|]*\| `check:tsgo-release-bundle` \|/,
    );
    expect(inventory).toMatch(
      /\| `rust\/release\/bundle` \| bundle \| declared \|[^|]*\|[^|]*\| `check:rust-release-bundle` \|/,
    );
    expect(inventory).toMatch(
      /\| `rust\/lane\/bundle` \| bundle \| declared \|[^|]*\|[^|]*\| `check:rust-lane-bundle` \|/,
    );
    expect(inventory).toMatch(
      /\| `rust\/omena-css\/h1-readiness` \| bundle \| declared \|[^|]*\|[^|]*\| `check:rust-omena-css-h1-readiness` \|/,
    );
    expect(inventory).toMatch(
      /\| `release\/release\/verify` \| bundle \| declared \|[^|]*\|[^|]*\| `release:verify` \|/,
    );
    expect(inventory).toMatch(
      /\| `release\/sync-server-version` \| command \| declared \|[^|]*\|[^|]*\| `@declared\/release\/sync-server-version` \|/,
    );
  });
