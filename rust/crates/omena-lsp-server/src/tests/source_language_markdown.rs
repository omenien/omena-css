...
#[test]
fn resolves_mdx_fenced_css_module_definition_to_opened_style_selector() -> TestResult {
    let source = r#"# Component notes
...
"#;
    let style = ".root { color: red; }\n";
    let source_uri = "file:///workspace-a/src/Card.mdx";
    let style_uri = "file:///workspace-a/src/Card.module.scss";
    let mut state = LspShellState::default();
    handle_lsp_message(
        &mut state,
    ...
    let document = state
        .document(source_uri)
        .ok_or_else(|| std::io::Error::other("MDX document is open"))?;
    assert_eq!(
        document
            .source_syntax_index
            .imported_style_bindings
    ...
    let definition_response = handle_lsp_message(
        ...
        json!({
            ...
            "params": {
                ...
                "position": parser_position_for_byte_offset(
                    source,
                    fixture_find(source, "styles.root", "MDX fence contains styles.root")?
                        + "styles.".len()
                        + 1,
                ),
            },
