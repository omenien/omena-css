...
pub(super) fn collect_scss_return_candidates_scanner_oracle(
    ...
    tokens
        ...
        .map(|(index, token)| {
            let return_text = scss_return_text_from_token_scanner_oracle(source, tokens, index);
            let return_inside_loop_control_flow =
                scss_return_is_inside_loop_control_flow_scanner_oracle(tokens, index);
            let return_loop_contexts =
                scss_return_loop_contexts_scanner_oracle(source, tokens, index);
            ...
            let return_condition =
                scss_return_condition_from_token_scanner_oracle(source, tokens, index);
...
fn scss_return_is_inside_loop_control_flow_scanner_oracle(
    tokens: &[LexedToken],
    return_index: usize,
) -> bool {
    !enclosing_scss_loop_blocks_scanner_oracle(tokens, return_index).is_empty()
}

fn scss_return_loop_contexts_scanner_oracle(
    source: &str,
    tokens: &[LexedToken],
    return_index: usize,
) -> Vec<ScssControlFlowLoopContext> {
    enclosing_scss_loop_blocks_scanner_oracle(tokens, return_index)
        .into_iter()
        .filter_map(|block| {
...
fn enclosing_scss_loop_blocks_scanner_oracle(
    tokens: &[LexedToken],
    return_index: usize,
) -> Vec<ScssBranchBlock> {
...
fn scss_return_text_from_token_scanner_oracle(
    source: &str,
    tokens: &[LexedToken],
    token_index: usize,
) -> Option<String> {
...
fn scss_return_condition_from_token_scanner_oracle(
    source: &str,
    tokens: &[LexedToken],
    return_index: usize,
) -> Option<ScssReturnCondition> {
    let branch_blocks = collect_scss_branch_blocks_scanner_oracle(source, tokens);
    ...
    let negated_condition_texts =
        previous_scss_branch_condition_texts_scanner_oracle(tokens, &branch_blocks, block_index);
...
fn collect_scss_branch_blocks_scanner_oracle(
    source: &str,
    tokens: &[LexedToken],
) -> Vec<ScssBranchBlock> {
    tokens
...
fn previous_scss_branch_condition_texts_scanner_oracle(
    tokens: &[LexedToken],
