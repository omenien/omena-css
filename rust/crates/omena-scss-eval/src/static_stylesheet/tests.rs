...
#[test]
fn static_less_evaluation_expands_property_name_interpolation() {
    let report = derive_static_stylesheet_module_evaluation(
        "@prop: color; .button { @{prop}: red; }",
        StyleDialect::Less,
    );
    assert!(report.is_some());
    let Some(report) = report else {
    ...
    assert!(!report.evaluated_css.contains("@prop:"));
    assert!(!report.evaluated_css.contains("@{prop}"));
...
#[test]
fn static_less_evaluation_expands_chained_class_selector_interpolation() {
    let report = derive_static_stylesheet_module_evaluation(
...
#[test]
fn static_less_evaluation_expands_chained_type_selector_interpolation() {
    let report = derive_static_stylesheet_module_evaluation(
        "@prefix: button; @suffix: primary; @{prefix}-@{suffix} { color: red; }",
        StyleDialect::Less,
    );
...
#[test]
fn static_less_evaluation_resolves_variable_indirection_aliases() {
    let report = derive_static_stylesheet_module_evaluation(
    ...
    assert!(!report.evaluated_css.contains("@name:"));
    assert!(!report.evaluated_css.contains("@color:"));
    assert!(!report.evaluated_css.contains("@alias:"));
    assert!(!report.evaluated_css.contains("@@name"));
    assert!(!report.evaluated_css.contains("@alias"));
    assert!(report.evaluated_css.contains("color: red"));
    assert!(report.oracle.all_legacy_declaration_values_preserved);
